...
func show_completed_picture() -> void:
	...
	QuestManager.update_quest("Rizal in Brussels", "Talk to filipino reformist", true)
	
	# 2. Hide the puzzle tiles so the full image is clear
	for tile in tiles:
		tile.visible = false

	# 3. Pause for 3 seconds so the player can see the victory screen
	await get_tree().create_timer(2.0).timeout
	
	# 4. Directly trigger the transition to the next level
	await Transitionlayer.start_smooth_transition("res://GUI/mainmenu/levels.tscn")
